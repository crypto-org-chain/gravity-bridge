...
pub async fn get_valset<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
    nonce: u64,
) -> Result<Option<Valset>, GravityError<S>> {
    let response = client
...
pub async fn get_oldest_unsigned_valsets<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
    address: Address,
) -> Result<Vec<Valset>, GravityError<S>> {
    let response = client
...
pub async fn get_latest_valset<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
) -> Result<Option<Valset>, GravityError<S>> {
    let response = client
...
pub async fn get_all_valset_confirms<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
    nonce: u64,
) -> Result<Vec<ValsetConfirmResponse>, GravityError<S>> {
    let request = client
...
pub async fn get_oldest_unsigned_transaction_batch<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
    address: Address,
) -> Result<Option<TransactionBatch>, GravityError<S>> {
    let request = client
        .unsigned_batch_txs(UnsignedBatchTxsRequest {
            address: address.to_string(),
        })
        .await?;
    let batches = extract_valid_batches::<S>(request.into_inner().batches);
...
pub async fn get_latest_transaction_batches<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
) -> Result<Vec<TransactionBatch>, GravityError<S>> {
    let request = client
        .batch_txs(BatchTxsRequest { pagination: None })
        .await?;
    Ok(extract_valid_batches::<S>(request.into_inner().batches))
}
...
fn extract_valid_batches<S: Signer>(batches: Vec<BatchTx>) -> Vec<TransactionBatch> {
    let mut valid_batches = Vec::new();
    for batch in batches {
        match TransactionBatch::from_proto::<S>(batch.clone()) {
...
pub async fn get_transaction_batch_signatures<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
    nonce: u64,
    contract_address: EthAddress,
) -> Result<Vec<BatchConfirmResponse>, GravityError<S>> {
    let request = client
...
pub async fn get_last_event_nonce<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
    address: Address,
...
pub async fn get_latest_logic_calls<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
) -> Result<Vec<LogicCall>, GravityError<S>> {
    let request = client
...
pub async fn get_logic_call_signatures<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
    invalidation_scope: Vec<u8>,
    invalidation_nonce: u64,
) -> Result<Vec<LogicCallConfirmResponse>, GravityError<S>> {
    let request = client
...
pub async fn get_oldest_unsigned_logic_call<S: Signer>(
    client: &mut GravityQueryClient<Channel>,
    address: Address,
) -> Result<Vec<LogicCall>, GravityError<S>> {
    let request = client
...
fn extract_valid_batches_test() {
    ...
    let valid_batches =
        extract_valid_batches::<LocalWallet>(vec![valid_batch, invalid_batch.clone()]);
    assert_eq!(valid_batches.len(), 1);
    assert_eq!(valid_batches.get(0).unwrap().nonce, 1);

    let should_be_empty = extract_valid_batches::<LocalWallet>(vec![invalid_batch]);
